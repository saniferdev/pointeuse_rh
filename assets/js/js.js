...
$(function() {
 "use strict";
  ...
  $('#dtBasicExample').DataTable({
    ...
    if( $('#num').val() != ''){
        $('select[name=badge]').val($('#num').val());
        $('.selectpicker__').selectpicker('refresh');
    }

    if( $('#site_').val() != ''){
        $('select[name=site]').val($('#site_').val());
        $('.selectpicker__').selectpicker('refresh');
    }

    if( $('#type_').val() != ''){
        $('select[name=type]').val($('#type_').val());
        $('.selectpicker_').selectpicker('refresh');
    }

    $(".valider").on( "click", function() {           
        $('#e').val("0");
        $(".chargement").css('display','inline-block');
        $( ".form_" ).submit();
    });

    $(".excel_").on( "click", function() {
        $('#e').val("1");            
        $(".chargement").css('display','inline-block');
        $( ".form_" ).submit();
        $(".chargement").hide();
